...
H5PEditor.widgets.interactiveVideo = H5PEditor.CurrikiInteractiveVideo = (function ($) {
   ...
   *
   * @class H5PEditor.CurrikiInteractiveVideo
   * @param {Object} parent
   * @param {Object} field
   * @param {Object} params
  ...
  function InteractiveVideoEditor(parent, field, params, setValue) {
    ...
    parent.on('stepChanged', function (event) {
      ...
      that.startGuidedTour(H5PEditor.CurrikiInteractiveVideo.GuidedTours.isOpen());
    });
   ...
   * @type {string}
   */
  InteractiveVideoEditor.clipboardKey = 'H5PEditor.CurrikiInteractiveVideo';
  /**
  ...
  InteractiveVideoEditor.prototype.setActive = function () {
    ...
    // Create new player.
    this.IV = new H5P.CurrikiInteractiveVideo({
      interactiveVideo: {
        video: {
          files: this.video,
          startScreenOptions: {
  ...
  InteractiveVideoEditor.prototype.addEndscreen = function (time, freshOnly) {
    ...
    this.params.endscreens.splice(i, 0, {
      time: time,
      label: H5P.CurrikiInteractiveVideo.humanizeTime(time) + ' ' + t('endscreen')
    });
   ...
   *
   * @param {H5P.CurrikiInteractiveVideoInteraction} interaction
  ...
  InteractiveVideoEditor.prototype.processInteraction = function (interaction, parameters) {
    ...
    if (eligibleForRequireCompletion) {
      new H5PEditor.CurrikiInteractiveVideo.RequireCompletion(self, interaction);
    }
  ...
  InteractiveVideoEditor.prototype.openInteractionDialog = function (interaction) {
    ...
    that.openForm(libraryField, interaction.$form[0], 'h5p-currikiinteractivevideo-editor');
   ...
   * @param {H5P.CurrikiInteractiveVideoInteraction} interaction Interaction
  ...
  InteractiveVideoEditor.prototype.startGuidedTour = function (force) {
    if (InteractiveVideoEditor.showGuidedTour) {
      H5PEditor.CurrikiInteractiveVideo.GuidedTours.start(this.currentTabIndex, force || false, t);
  ...
  InteractiveVideoEditor.prototype.appendTo = function ($wrapper) {
    ...
    if ($libwrap.hasClass('libwrap')) {
      $libwrap.addClass('h5p-currikiinteractivevideo-editor');
    }
  ...
  InteractiveVideoEditor.prototype.remove = function () {
    ...
    H5PEditor.CurrikiInteractiveVideo.GuidedTours.remove();
    this.$item.remove();
  };
  ...
  var t = InteractiveVideoEditor.t = function (key, vars) {
    return H5PEditor.t('H5PEditor.CurrikiInteractiveVideo', key, vars);
  };
