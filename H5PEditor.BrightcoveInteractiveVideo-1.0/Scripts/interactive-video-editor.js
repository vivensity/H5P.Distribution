...
H5PEditor.widgets.interactiveVideo = H5PEditor.BrightcoveInteractiveVideo = (function ($) {
   ...
   *
   * @class H5PEditor.BrightcoveInteractiveVideo
   * @param {Object} parent
   * @param {Object} field
   * @param {Object} params
  ...
  function InteractiveVideoEditor(parent, field, params, setValue) {
    ...
    parent.on('stepChanged', function (event) {
      ...
      that.startGuidedTour(H5PEditor.BrightcoveInteractiveVideo.GuidedTours.isOpen());
    });
   ...
   * @type {string}
   */
  InteractiveVideoEditor.clipboardKey = 'H5PEditor.BrightcoveInteractiveVideo';
  /**
  ...
  InteractiveVideoEditor.prototype.addEndscreen = function (time, freshOnly) {
    ...
    this.params.endscreens.splice(i, 0, {
      time: time,
      label: H5P.BrightcoveInteractiveVideo.humanizeTime(time) + ' ' + t('endscreen')
    });
   ...
   *
   * @param {H5P.BrightcoveInteractiveVideoInteraction} interaction
  ...
  InteractiveVideoEditor.prototype.processInteraction = function (interaction, parameters) {
    ...
    if (eligibleForRequireCompletion) {
      new H5PEditor.BrightcoveInteractiveVideo.RequireCompletion(self, interaction);
    }
  ...
  InteractiveVideoEditor.prototype.openInteractionDialog = function (interaction) {
    ...
    that.openForm(libraryField, interaction.$form[0], 'h5p-brightcoveinteractivevideo-editor');
   ...
   * @param {H5P.BrightcoveInteractiveVideoInteraction} interaction Interaction
  ...
  InteractiveVideoEditor.prototype.startGuidedTour = function (force) {
    if (InteractiveVideoEditor.showGuidedTour) {
      H5PEditor.BrightcoveInteractiveVideo.GuidedTours.start(this.currentTabIndex, force || false, t);
  ...
  InteractiveVideoEditor.prototype.appendTo = function ($wrapper) {
    ...
    if ($libwrap.hasClass('libwrap')) {
      $libwrap.addClass('h5p-brightcoveinteractivevideo-editor');
    }
  ...
  InteractiveVideoEditor.prototype.remove = function () {
    ...
    H5PEditor.BrightcoveInteractiveVideo.GuidedTours.remove();
    this.$item.remove();
  };
  ...
  var t = InteractiveVideoEditor.t = function (key, vars) {
    return H5PEditor.t('H5PEditor.BrightcoveInteractiveVideo', key, vars);
  };
