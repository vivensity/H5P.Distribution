...
(function ($, ImageHotspots) {
  ...
  ImageHotspots.Hotspot = function (config, options, id, isSmallDeviceCB, parent) {
    ...
    if (this.config.content[0].library == 'H5P.Link 1.3') {
     
      this.$element = $('<button/>', {
        'class': 'h5p-image-hotspot h5p-image-linkh5p',
        'role': 'button',
        'tabindex': 0,
        'aria-haspopup': true,
      });
  ...
  ImageHotspots.Hotspot.prototype.appendTo = function ($container) {
    ...
    if (this.config.content[0].library == 'H5P.Link 1.3') {
      // var url = this.config.content[0].params.text
    
      this.$element[0].innerHTML=`<a href="${this.config.content[0].params.linkWidget.protocol+this.config.content[0].params.linkWidget.url}" target="_blank">${this.config.content[0].params.title}</a>`
